...
/* $Id: PipelineFactoryTestCase.java 924666 2010-03-18 08:26:30Z jeremias $ */
...
public class PipelineFactoryTestCase extends TestCase {
	 ...
	 */
	@Test
	public void testPipelineFactoryPlain() {
		final MockImageContext imageContext = MockImageContext
				.newSafeInstance();
		final ImageManager manager = imageContext.getImageManager();
		final PipelineFactory pFactory = new PipelineFactory(manager);

		// Input is some TIFF file
		final ImageInfo imageInfo = new ImageInfo("test:tiff", "image/tiff");

		// We want a G2D image
		final ImageFlavor targetFlavor = ImageFlavor.GRAPHICS2D;
		...
		// penalty for internal TIFF implementation (fallback role) is 1000 + 10
		// for the conversion
		...
		ImageProviderPipeline[] candidates = pFactory
				.determineCandidatePipelines(imageInfo,
						new ImageFlavor[] { targetFlavor });
		assertEquals(1, candidates.length);

		// Now add another implementation that poses as TIFF loader
		imageContext.getImageManager().getRegistry()
				.registerLoaderFactory(new MockImageLoaderFactoryTIFF());

		candidates = pFactory.determineCandidatePipelines(imageInfo,
				targetFlavor);
		assertEquals(3, candidates.length);
		// 3 because the mock impl provides Buffered- and RenderedImage
		// capabilities
		...
		assertEquals(ImageFlavor.GRAPHICS2D, pipeline.getTargetFlavor());
		if (pipeline.toString().indexOf(
				MockImageLoaderFactoryTIFF.class.getName()) < 0) {
			fail("Chose the wrong pipeline: " + pipeline.toString());
		}
		if (pipeline.toString().indexOf("ImageConverterBitmap2G2D") < 0) {
			fail("Chose the wrong pipeline: " + pipeline.toString());
	...
	/**
	 * Similar test as above but here we take raw CCITT loading into
	 * consideration, too.
	 * 
	 * @if an error occurs
	 */
	@Test
	public void testPipelineFactoryImageInfoDependency() {
		final MockImageContext imageContext = MockImageContext
				.newSafeInstance();
		final ImageManager manager = imageContext.getImageManager();
		final PipelineFactory pFactory = new PipelineFactory(manager);
		...
		imageInfo.getCustomObjects().put("TIFF_COMPRESSION",
				TIFFImage.COMP_FAX_G4_2D);

		// We want either a G2D image or a raw CCITT image
		final ImageFlavor[] targetFlavors = new ImageFlavor[] {
				ImageFlavor.GRAPHICS2D, ImageFlavor.RAW_CCITTFAX };

		ImageProviderPipeline[] candidates = pFactory
				.determineCandidatePipelines(imageInfo, targetFlavors);
		...
		// Now, we set this to a multi-strip TIFF which should disable the raw
		// loader
	...
	/**
	 * Similar test as above but now we're playing with additional penalties in
	 * the registry.
	 * 
	 * @if an error occurs
	 */
	@Test
	public void testPipelineFactoryAdditionalPenalty() {
		final MockImageContext imageContext = MockImageContext
				.newSafeInstance();
		final ImageManager manager = imageContext.getImageManager();
		final PipelineFactory pFactory = new PipelineFactory(manager);
		...
		final ImageImplRegistry registry = imageContext.getImageManager()
				.getRegistry();
		...
		final ImageProviderPipeline pipeline = manager
				.choosePipeline(candidates);
		...
		assertNotNull(candidates);
		assertEquals(1, candidates.length);
		// While earlier 2 candidates were returned, here we only get 1 because
		// of the infinite
		// penalty.
	}
