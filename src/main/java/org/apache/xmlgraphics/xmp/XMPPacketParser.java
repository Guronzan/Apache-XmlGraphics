 ...
 * first XMP packet it finds and parses it.
 * <p>
 * Important: Before you use this class to look for an XMP packet in some random
 * file, please read the chapter on "Scanning Files for XMP Packets" in the XMP
 * specification!
 */
@Slf4j
public final class XMPPacketParser {
    ...
    static {
        try {
            ...
            throw new RuntimeException(
                    "Incompatible JVM! US-ASCII encoding not supported.");
        }
    }
     ...
     */
    public static Metadata parse(final InputStream inputIn) throws IOException,
            TransformerException {
        final InputStream in;
        if (!inputIn.markSupported()) {
            in = new BufferedInputStream(inputIn);
        } else {
            in = inputIn;
        }

        final boolean foundXMP = skipAfter(in, PACKET_HEADER);
        if (!foundXMP) {
            return null;
        }
        ...
        // TODO think about not buffering this but for example, parse in another
        // thread
        ...
        try (final ByteArrayOutputStream baout = new ByteArrayOutputStream()) {
            ...
            final Metadata metadata = XMPParser.parseXMP(new StreamSource(
                    new ByteArrayInputStream(baout.toByteArray())));
            return metadata;
        }
    }

    private static boolean skipAfter(final InputStream in, final byte[] match)
            throws IOException {
