 ...
 * abstract, but it implements a lot of the <tt>Graphics2D</tt> method in a way
 * that concrete implementations can reuse.
 *
 * This class uses a <tt>GraphicContext</tt> to store the state of its various
 * attributes that control the rendering, such as the current <tt>Font</tt>,
 * <tt>Paint</tt> or clip.
 *
 * Concrete implementations can focus on implementing the rendering methods,
 * such as <tt>drawShape</tt>. As a convenience, rendering methods that can be
 * expressed with other rendering methods (e.g., <tt>drawRect</tt> can be
 * expressed as <tt>draw(new Rectangle(..))</tt>), are implemented by
 * <tt>AbstractGraphics2D</tt>
 *
...
public abstract class AbstractGraphics2D extends Graphics2D implements
    ...
    public void drawRoundRect(final int x, final int y, final int width,
        ...
        final RoundRectangle2D rect = new RoundRectangle2D.Float(x, y, width,
    ...
    public void drawArc(final int x, final int y, final int width,
        ...
        final Arc2D arc = new Arc2D.Float(x, y, width, height, startAngle,
                arcAngle, Arc2D.OPEN);
        draw(arc);
    }
    ...
    public boolean drawImage(final Image img, final AffineTransform xform,
        ...
        if (xform.getDeterminant() != 0) {
            ...
            final AffineTransform savTransform = new AffineTransform(
                    this.gc.getTransform());
            this.gc.transform(xform);
            retVal = drawImage(img, 0, 0, null);
     ...
     */
    @Override
    public void drawImage(final BufferedImage inImg, final BufferedImageOp op,
            final int x, final int y) {
        final BufferedImage img = op.filter(inImg, null);
    ...
    public void setRenderingHints(final Map<?, ?> hints) {
        this.gc.setRenderingHints((Map<Key, ?>) hints);
    }
    ...
    public void addRenderingHints(final Map<?, ?> hints) {
        this.gc.addRenderingHints((Map<Key, ?>) hints);
    }
