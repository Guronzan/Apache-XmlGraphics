 ...
 *
 * @version $Id: AbstractPSDocumentGraphics2D.java 1345683 2012-06-03 14:50:33Z
 *          gadams $
...
public abstract class AbstractPSDocumentGraphics2D extends PSGraphics2D {
     ...
     *            and not the font.
     */
    AbstractPSDocumentGraphics2D(final boolean textAsShapes) {
        super(textAsShapes);
    }
     ...
     *             stream
     */
    public AbstractPSDocumentGraphics2D(final boolean textAsShapes,
    ...
    /**
     * Set the dimensions of the document that will be drawn. This is useful if
     * the dimensions of the document are different from the PostScript document
     * that is to be created. The result is scaled so that the document fits
     * correctly inside the PostScript document.
     * 
     * @param w
     *            the width of the page
     * @param h
     *            the height of the page
     */
    public void setViewportDimension(final float w, final float h) {
        this.viewportHeight = h;
        /*
         * if (w != this.width || h != this.height) { gen.concatMatrix(width /
         * w, 0, 0, height / h, 0, 0); }
         */
    }

    /**
     * Set the background of the PostScript document. This is used to set the
     * background for the PostScript document Rather than leaving it as the
     * default white.
     * 
     * @param col
    ...
    public void setBackgroundColor(final Color col) {
        /** (todo) Implement this */
        /*
         * Color c = col; PDFColor currentColour = new PDFColor(c.getRed(),
         * c.getGreen(), c.getBlue()); currentStream.write("q\n");
     ...
     * Override to add your own ProcSets if so desired.
     * 
     * @throws IOException
     *             In case an I/O error occurs
     */
    ...
    protected void startPage() throws IOException {
        ...
        if ((this.viewportWidth != this.width || this.viewportHeight != this.height)
            ...
            at = new AffineTransform(this.width / this.viewportWidth, 0, 0, -1
                    * (this.height / this.viewportHeight), 0, this.height);
        } else {
            at = new AffineTransform(1, 0, 0, -1, 0, this.height);
        }
        // Do not use concatMatrix, since it alters PSGenerator current state
        // gen.concatMatrix(at);
        this.gen.writeln(this.gen.formatMatrix(at) + " "
                + this.gen.mapCommand("concat"));
    ...
    public void finish() throws IOException {
        ...
        this.gen.writeDSCComment(DSCConstants.PAGES, this.pagecount);
     ...
     */
    public AbstractPSDocumentGraphics2D(final AbstractPSDocumentGraphics2D g) {
        super(g);
    }
