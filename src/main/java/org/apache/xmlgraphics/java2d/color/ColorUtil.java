...
public final class ColorUtil {
    ...
    /**
     * Converts an arbitrary {@link Color} to a plain sRGB color doing the
     * conversion at the best possible conversion quality.
     * 
     * @param col
     *            the original color
     * @return the sRGB equivalent
     */
    public static Color toSRGBColor(final Color col) {
        if (col.getColorSpace().isCS_sRGB()) {
            return col; // Don't convert if already sRGB to avoid conversion
                        // differences
        }
        float[] comps = col.getColorComponents(null);
        final float[] srgb = col.getColorSpace().toRGB(comps);
        comps = col.getComponents(null);
        final float alpha = comps[comps.length - 1];
        return new Color(srgb[0], srgb[1], srgb[2], alpha);
    }

    /**
     * Checks if two colors are the same color. This check is much more
     * restrictive than {@link Color#equals(Object)} in that it doesn't only
     * check if both colors result in the same sRGB value. For example, if two
     * colors not of the same exact class are compared, they are treated as not
     * the same.
     * <p>
     * Note: At the moment, this method only supports {@link Color} and
     * {@link ColorWithAlternatives} only. Other subclasses of {@link Color} are
     * checked only using the {@link Color#equals(Object)} method.
     * 
     * @param col1
     *            the first color
     * @param col2
     ...
     */
    public static boolean isSameColor(final Color col1, final Color col2) {
        // Check fallback sRGB values first, then go into details
        if (!col1.equals(col2)) {
            return false;
        }

        // Consider same-ness only between colors of the same class (not
        // subclasses)
        // but consider a ColorWithAlternatives without alternatives to be the
        // same as a Color.
        ...
        final float[] comps1 = col1.getComponents(null);
        final float[] comps2 = col2.getComponents(null);
        ...
        if (col1 instanceof ColorWithAlternatives
            ...
            final ColorWithAlternatives ca1 = (ColorWithAlternatives) col1;
            final ColorWithAlternatives ca2 = (ColorWithAlternatives) col2;
