...
/**
 * This class defines the CIE L*a*b* (CIE 1976) color space. Valid values for L*
 * are between 0 and 100, for a* and b* between -127 and +127.
 * 
...
public class CIELabColorSpace extends ColorSpace {
    ...
    // CIE XYZ tristimulus values of the reference white point: Observer= 2
     ...
     * 
     * @param whitePoint
     *            the white point in XYZ coordinates (valid values: 0.0f to
     *            1.0f, although values slightly larger than 1.0f are common)
     */
    ...
    private void checkNumComponents(final float[] colorvalue) {
    ...
    // Note: the conversion functions used here were mostly borrowed from Apache
    // Commons Sanselan
    ...
    public float[] fromRGB(final float[] rgbvalue) {
        ...
        final float[] xyz = sRGB.toCIEXYZ(rgbvalue);
    ...
    public float[] toCIEXYZ(final float[] colorvalue) {
        ...
        final float l = denormalize(colorvalue[0], 0);
        final float a = denormalize(colorvalue[1], 1);
        final float b = denormalize(colorvalue[2], 2);

        return toCIEXYZNative(l, a, b);
    }

    /**
     * Transforms a color value assumed to be in this {@link ColorSpace} into
     * the CS_CIEXYZ conversion color space. This method uses component values
     * in CIE Lab's native color ranges rather than the normalized values
     * between 0 and 1.
     * 
     * @param l
     *            the L* component (values between 0 and 100)
     * @param a
     *            the a* component (usually between -128 and +128)
     * @param b
    ...
    public float[] toCIEXYZNative(final float l, final float a, final float b) {
        ...
        final float x = (float) (this.wpX * varX / 100);
        final float y = (float) (this.wpY * varY / 100);
        final float z = (float) (this.wpZ * varZ / 100);
    ...
    private float getNativeValueRange(final int component) {
        return getMaxValue(component) - getMinValue(component);
    }

    private float normalize(final float value, final int component) {
        return (value - getMinValue(component))
                / getNativeValueRange(component);
    }

    private float denormalize(final float value, final int component) {
        return value * getNativeValueRange(component) + getMinValue(component);
    }

    /**
     * Converts normalized (0..1) color components to CIE L*a*b*'s native value
     * range.
     * 
     * @param comps
    ...
    /**
     * Creates a {@link Color} instance from color values usually used by the
     * L*a*b* color space by scaling them to the 0.0..1.0 range expected by
     * Color's constructor.
     * 
     * @param colorvalue
     *            the original color values (native value range, i.e. not
     ...
     *            the L* component (values between 0 and 100)
     * @param a
     *            the a* component (usually between -128 and +127)
     * @param b
     *            the b* component (usually between -128 and +127)
     * @param alpha
     *            the alpha component (values between 0 and 1)
     * @return the requested color instance
     */
    public Color toColor(final float l, final float a, final float b,
            final float alpha) {
