...
public final class ColorProfileUtil {
     ...
     */
    public static String getICCProfileDescription(final ICC_Profile profile) {
        final byte[] data = profile
                .getData(ICC_Profile.icSigProfileDescriptionTag);
        if (data == null) {
            return null;
        } else {
            // Info on the data format: http://www.color.org/ICC-1_1998-09.PDF
            int length = data[8] << 3 * 8 | data[9] << 2 * 8 | data[10] << 8
                    | data[11];
    ...
    /**
     * Indicates whether a given color profile is identical to the default sRGB
     * profile provided by the Java class library.
     * 
     * @param profile
     *            the color profile to check
     * @return true if it is the default sRGB profile
     */
    public static boolean isDefaultsRGB(final ICC_Profile profile) {
    ...
    /**
     * Proxy method for {@link ICC_Profile#getInstance(byte[])} that properly
     * synchronizes the call to avoid a potential race condition.
     * 
     * @param data
     ...
     */
    public static ICC_Profile getICC_Profile(final byte[] data) {
    ...
    /**
     * Proxy method for {@link ICC_Profile#getInstance(int)} that properly
     * synchronizes the call to avoid a potential race condition.
     * 
     * @param colorSpace
     *            the type of color space to create a profile for. The specified
     *            type is one of the color space constants defined in the
     *            {@link ColorSpace} class.
     * @return an {@link ICC_Profile} instance corresponding to the specified
     *         {@code ColorSpace}
     * @throws IllegalArgumentException
     *             if {@code colorSpace} is not one of the predefined types
     */
    public static ICC_Profile getICC_Profile(final int colorSpace) {
        synchronized (ICC_Profile.class) {
     ...
     */
    public static ICC_Profile getICC_Profile(final InputStream in)
            throws IOException {
        synchronized (ICC_Profile.class) {
    ...
    /**
     * Proxy method for {@link ICC_Profile#getInstance(java.lang.String)} that
     * properly synchronizes the call to avoid a potential race condition.
     * 
     * @param fileName
     ...
     *         specified file
     * @throws IOException
     *             if the file cannot be opened, or an I/O error occurs while
     *             reading the stream
     * @throws IllegalArgumentException
     *             if the stream does not contain valid ICC Profile data
     ...
     */
    public static ICC_Profile getICC_Profile(final String fileName)
            throws IOException {
        synchronized (ICC_Profile.class) {
