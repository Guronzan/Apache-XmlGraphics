...
public class FormatRed extends AbstractRed {
    ...
    public static CachableRed construct(final CachableRed src,
            final ColorModel cm) {
    ...
    public FormatRed(final CachableRed cr, final SampleModel sm) {
        super(cr, cr.getBounds(), makeColorModel(cr, sm), sm, cr
                .getTileGridXOffset(), cr.getTileGridYOffset(), null);
    }

    public FormatRed(final CachableRed cr, final ColorModel cm) {
        super(cr, cr.getBounds(), cm, makeSampleModel(cr, cm), cr
                .getTileGridXOffset(), cr.getTileGridYOffset(), null);
    }
    ...
    public WritableRaster copyData(final WritableRaster wr) {
        ...
        final CachableRed cr = getSource();
        final ColorModel srcCM = cr.getColorModel();
        SampleModel srcSM = cr.getSampleModel();
        srcSM = srcSM
                .createCompatibleSampleModel(wr.getWidth(), wr.getHeight());
        WritableRaster srcWR;
        srcWR = Raster.createWritableRaster(srcSM,
                new Point(wr.getMinX(), wr.getMinY()));
        getSource().copyData(srcWR);

        final BufferedImage srcBI = new BufferedImage(srcCM,
                srcWR.createWritableTranslatedChild(0, 0),
                srcCM.isAlphaPremultiplied(), null);
        final BufferedImage dstBI = new BufferedImage(cm,
    ...
    public static SampleModel makeSampleModel(final CachableRed cr,
            final ColorModel cm) {
    ...
    public static ColorModel makeColorModel(final CachableRed cr,
        ...
        if (sm instanceof ComponentSampleModel) {
            ...
            if (bands == 4) {
                return new DirectColorModel(cs, bits, masks[0], masks[1],
                        masks[2], masks[3], preMult, dt);
            } else if (bands == 3) {
                return new DirectColorModel(cs, bits, masks[0], masks[1],
                        masks[2], 0x0, preMult, dt);
            } else {
                throw new IllegalArgumentException(
