...
public class ImageWriterRegistry {
    ...
    private final Map<String, List<ImageWriter>> imageWriterMap = new HashMap<>();
    private final Map<Object, Object> preferredOrder;
    ...
    private void setup() {
        final Iterator<ImageWriter> iter = Service.providers(ImageWriter.class);
        while (iter.hasNext()) {
            register(iter.next());
        }
    }
    ...
    public void register(final ImageWriter writer) {
        List<ImageWriter> entries = this.imageWriterMap.get(writer
                .getMIMEType());
        if (entries == null) {
        ...
        final ListIterator<ImageWriter> li = entries.listIterator();
        while (li.hasNext()) {
            final ImageWriter w = li.next();
    ...
    public ImageWriter getWriterFor(final String mime) {
        final List<ImageWriter> entries = this.imageWriterMap.get(mime);
        if (entries == null) {
            return null;
        }
        for (final ImageWriter imageWriter : entries) {
