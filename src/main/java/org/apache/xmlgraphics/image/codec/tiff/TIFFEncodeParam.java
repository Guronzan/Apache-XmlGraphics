...
public class TIFFEncodeParam implements ImageEncodeParam {
     ...
     *
     */
    private static final long serialVersionUID = 9029672144635104065L;
    ...
    /**
     * Code for original JPEG-in-TIFF compression which has been depricated (for
     * many good reasons) in favor of Tech Note 2 JPEG compression (compression
     * scheme 7).
     * <p>
     * <b>Not supported.</b>
     */
    ...
    private Iterator<Object> extraImages;
    ...
    /**
     * Constructs a TIFFEncodeParam object with default values for all
     * parameters.
     */
     ...
     * written out. Currently only PackBits, JPEG, and DEFLATE compression
     * schemes are supported.
     *
     * <p>
     * If <code>compression</code> is set to any value but
     * <code>COMPRESSION_NONE</code> and the <code>OutputStream</code> supplied
     * to the <code>ImageEncoder</code> is not a
     * <code>SeekableOutputStream</code>, then the encoder will use either a
     * temporary file or a memory cache when compressing the data depending on
     * whether the file system is accessible. Compression will therefore be more
     * efficient if a <code>SeekableOutputStream</code> is supplied.
     *
     * @param compression
    ...
    public void setCompression(final int compression) {
        ...
        default:
            throw new RuntimeException(
                    PropertyUtil.getString("TIFFEncodeParam0"));
        }
    ...
    /**
     * Sets the dimensions of the tiles to be written. If either value is
     * non-positive, the encoder will use a default value.
     *
     * <p>
     * If the data are being written as tiles, i.e.,
     * <code>getWriteTiled()</code> returns <code>true</code>, then the default
     * tile dimensions used by the encoder are those of the tiles of the image
     * being encoded.
     *
     * <p>
     * If the data are being written as strips, i.e.,
     * <code>getWriteTiled()</code> returns <code>false</code>, the width of
     * each strip is always the width of the image and the default number of
     * rows per strip is 8.
     *
     * @param tileWidth
     *            The tile width; ignored if strips are used.
     * @param tileHeight
    ...
    /**
     * Sets an <code>Iterator</code> of additional images to be written after
     * the image passed as an argument to the <code>ImageEncoder</code>. The
     * methods on the supplied <code>Iterator</code> must only be invoked by the
     * <code>ImageEncoder</code> which will exhaust the available values unless
     * an error occurs.
     *
     * <p>
     * The value returned by an invocation of <code>next()</code> on the
     * <code>Iterator</code> must return either a <code>RenderedImage</code> or
     * an <code>Object[]</code> of length 2 wherein the element at index zero is
     * a <code>RenderedImage</code> amd the other element is a
     * <code>TIFFEncodeParam</code>. If no <code>TIFFEncodeParam</code> is
     * supplied in this manner for an additional image, the parameters used to
     * create the <code>ImageEncoder</code> will be used. The extra image
     * <code>Iterator</code> set on any <code>TIFFEncodeParam</code> of an
     * additional image will in all cases be ignored.
     */
    public synchronized void setExtraImages(final Iterator<Object> extraImages) {
        this.extraImages = extraImages;
    }

    /**
     * Returns the additional image <code>Iterator</code> specified via
     * <code>setExtraImages()</code> or <code>null</code> if none was supplied
     * or if a <code>null</code> value was supplied.
     */
    public synchronized Iterator<Object> getExtraImages() {
        return this.extraImages;
    }

    /**
     * Sets the compression level for DEFLATE-compressed data which should
     * either be <code>java.util.Deflater.DEFAULT_COMPRESSION</code> or a value
     * in the range [1,9] where larger values indicate more compression. The
     * default setting is <code>Deflater.DEFAULT_COMPRESSION</code>. This
    ...
    public void setDeflateLevel(final int deflateLevel) {
        if (deflateLevel < 1 && deflateLevel > 9
            ...
            throw new RuntimeException(
                    PropertyUtil.getString("TIFFEncodeParam1"));
        }
     ...
     * compression type is JPEG. The default value is <code>true</code>. This
     * flag is ignored if the compression type is not JPEG.
     */
     ...
     * Directory (IFD). Fields with tags equal to the tag of any automatically
     * generated fields are ignored. No error checking is performed with respect
     * to the validity of the field contents or the appropriateness of the field
     * for the image being encoded.
     *
     * @param extraFields
     *            An array of extra fields; the parameter is copied by
     *            reference.
     */
