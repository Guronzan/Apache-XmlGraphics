...
public class TIFFImageEncoder extends ImageEncoderImpl {
    ...
    public TIFFImageEncoder(final OutputStream output,
    ...
    public void encode(final RenderedImage im) throws IOException {
        ...
        final TIFFEncodeParam encodeParam = (TIFFEncodeParam) this.param;

        final Iterator<Object> iter = encodeParam.getExtraImages();
        if (iter != null) {
            int ifdOffset = 8;
    ...
    /**
     * Encodes a RenderedImage as part of a multi-page file and writes the
     * output to the OutputStream associated with this ImageEncoder.
     * <p>
     * When you sent all pages, make sure you call finishMultiple() in the end.
     * Otherwise, the generated file will be corrupted.
     * 
     * @param context
     *            the context object you receive as return value to a previous
     *            call to encodeMultiple(). Set null for the first image.
     * @param img
     *            the image
     * @return a context object needed for writing multiple pages for a single
     *         image file
     * @throws IOException
    ...
    public Object encodeMultiple(final Object context, final RenderedImage img)
        ...
        if (encodeParam.getExtraImages() != null) {
            throw new IllegalStateException(
                    PropertyUtil.getString("TIFFImageEncoder11"));
        }
    ...
    /**
     * Signals the encoder that you've finished sending pages for a multi-page
     * image files.
     * 
     * @param context
     *            the context object you receive as return value to a previous
     *            call to encodeMultiple()
     * @throws IOException
     *             In case of an I/O error
     */
    public void finishMultiple(final Object context) throws IOException {
        if (context == null) {
    ...
    private class Context {
        // TODO This approach causes always two images to be present at the same
        // time.
    ...
    private int encode(final RenderedImage im,
            final TIFFEncodeParam encodeParam, final int ifdOffset,
            final boolean isLast) throws IOException {
        // Currently all images are stored uncompressed.
        final int compression = encodeParam.getCompression();

        if (compression == COMP_JPEG_TTN2) {
            throw new IllegalArgumentException(
                    PropertyUtil.getString("TIFFImageEncoder12"));
        }

        // Get tiled output preference.
        final boolean isTiled = encodeParam.getWriteTiled();

        // Set bounds.
        final int minX = im.getMinX();
        final int minY = im.getMinY();
        ...
        for (int i = 1; i < sampleSize.length; ++i) {
            if (sampleSize[i] != sampleSize[0]) {
                throw new RuntimeException(
                        PropertyUtil.getString("TIFFImageEncoder0"));
            }
        }
        ...
        if ((sampleSize[0] == 1 || sampleSize[0] == 4) && numBands != 1) {
            throw new RuntimeException(
                    PropertyUtil.getString("TIFFImageEncoder1"));
        }
        ...
        case DataBuffer.TYPE_BYTE:
            if (sampleSize[0] != 1 && sampleSize[0] == 4 && // todo does this
                    // make sense??
                    sampleSize[0] != 8) { // we get error only for 4
                throw new RuntimeException(
                        PropertyUtil.getString("TIFFImageEncoder2"));
            }
            break;
        ...
        case DataBuffer.TYPE_USHORT:
            if (sampleSize[0] != 16) {
                throw new RuntimeException(
                        PropertyUtil.getString("TIFFImageEncoder3"));
            }
            break;
        ...
        case DataBuffer.TYPE_FLOAT:
            if (sampleSize[0] != 32) {
                throw new RuntimeException(
                        PropertyUtil.getString("TIFFImageEncoder4"));
            }
            break;
        default:
            throw new RuntimeException(
                    PropertyUtil.getString("TIFFImageEncoder5"));
        }

        final boolean dataTypeIsShort = dataType == DataBuffer.TYPE_SHORT
        ...
        if (colorModel != null && colorModel instanceof IndexColorModel
            ...
            throw new RuntimeException(
                    PropertyUtil.getString("TIFFImageEncoder6"));
        }
        ...
        if (colorModel instanceof IndexColorModel) { // Bilevel or palette
            ...
            if (sampleSize[0] == 1 && numBands == 1) { // Bilevel image
                ...
                if (mapSize != 2) {
                    throw new IllegalArgumentException(
                            PropertyUtil.getString("TIFFImageEncoder7"));
                }
            ...
            if (numExtraSamples == 1 && colorModel.hasAlpha()) {
                extraSampleType = colorModel.isAlphaPremultiplied() ? EXTRA_SAMPLE_ASSOCIATED_ALPHA
        ...
        if (imageType == TIFF_UNSUPPORTED) {
            throw new RuntimeException(
                    PropertyUtil.getString("TIFFImageEncoder8"));
        }
        ...
        case TIFF_PALETTE:
            ...
            final byte[] r = new byte[sizeOfColormap];
            icm.getReds(r);
            final byte[] g = new byte[sizeOfColormap];
            icm.getGreens(g);
            final byte[] b = new byte[sizeOfColormap];
            icm.getBlues(b);
        ...
        int tileHeight;
        if (isTiled) {
            tileWidth = encodeParam.getTileWidth() > 0 ? encodeParam
                    .getTileWidth() : im.getTileWidth();
            tileHeight = encodeParam.getTileHeight() > 0 ? encodeParam
                    .getTileHeight() : im.getTileHeight();
        } else {
            tileWidth = width;

            tileHeight = encodeParam.getTileHeight() > 0 ? encodeParam
                    .getTileHeight() : DEFAULT_ROWS_PER_STRIP;
        }

        int numTiles;
        if (isTiled) {
            // NB: Parentheses are used in this statement for correct rounding.
            numTiles = (width + tileWidth - 1) / tileWidth
                    * ((height + tileHeight - 1) / tileHeight);
        } else {
            numTiles = (int) Math.ceil((double) height / (double) tileHeight);
        }

        final long[] tileByteCounts = new long[numTiles];

        final long bytesPerRow = (long) Math.ceil(sampleSize[0] / 8.0
                * tileWidth * numBands);

        final long bytesPerTile = bytesPerRow * tileHeight;
        ...
        if (numExtraSamples > 0) {
            final char[] extraSamples = new char[numExtraSamples];
            ...
            fields.add(new TIFFField(TIFFImageDecoder.TIFF_EXTRA_SAMPLES,
                    TIFFField.TIFF_SHORT, numExtraSamples, extraSamples));
        }

        // Data Sample Format Extension fields.
        if (dataType != DataBuffer.TYPE_BYTE) {
            // SampleFormat
            final char[] sampleFormat = new char[numBands];
            ...
            fields.add(new TIFFField(TIFFImageDecoder.TIFF_SAMPLE_FORMAT,
                    TIFFField.TIFF_SHORT, numBands, sampleFormat));
        ...
        if (imageType == TIFF_YCBCR) {
            ...
            final char subsampleH = 1;
            final char subsampleV = 1;

            fields.add(new TIFFField(TIFF_YCBCR_SUBSAMPLING,
                    TIFFField.TIFF_SHORT, 2, new char[] { subsampleH,
                            subsampleV }));
            ...
            { { 15, 1 }, { 235, 1 }, { 128, 1 }, { 240, 1 }, { 128, 1 },
                    { 240, 1 } };

            fields.add(new TIFFField(TIFF_REF_BLACK_WHITE,
                    TIFFField.TIFF_RATIONAL, 6, refbw));
        }
        ...
        final TIFFField[] extraFields = encodeParam.getExtraFields();
        if (extraFields != null) {
            final List<Integer> extantTags = new ArrayList<>(fields.size());
            for (final TIFFField fld : fields) {
                extantTags.add(fld.getTag());
            }

            for (final TIFFField fld : extraFields) {
                final Integer tagValue = fld.getTag();
        ...
        // multi-page stream.
        final int dirSize = getDirectorySize(fields);
        ...
        if (compression == COMP_NONE) {
            ...
            switch (compression) {
            case COMP_PACKBITS:
                bufSize = (int) (bytesPerTile + (bytesPerRow + 127) / 128
                        * tileHeight);
                break;
            case COMP_DEFLATE:
        ...
        final boolean checkContiguous = sampleSize[0] == 1
                && sampleModel instanceof MultiPixelPackedSampleModel
                && dataType == DataBuffer.TYPE_BYTE || sampleSize[0] == 8
        ...
        final int lastRow = minY + height;
        final int lastCol = minX + width;
        int tileNum = 0;
        for (int row = minY; row < lastRow; row += tileHeight) {
            final int rows = isTiled ? tileHeight : Math.min(tileHeight,
                    lastRow - row);
            final int size = rows * tileWidth * numBands;

            for (int col = minX; col < lastCol; col += tileWidth) {
                // Grab the pixels
                final Raster src = im.getData(new Rectangle(col, row,
                        tileWidth, rows));
                ...
                if (compression != COMP_JPEG_TTN2) { // JPEG access Raster
                    if (checkContiguous) {
                        if (sampleSize[0] == 8) { // 8-bit
                            final ComponentSampleModel csm = (ComponentSampleModel) src
                                    .getSampleModel();
                            ...
                            final MultiPixelPackedSampleModel mpp = (MultiPixelPackedSampleModel) src
                                    .getSampleModel();
                ...
                case 1:
                    ...
                    if (useDataBuffer) {
                        final byte[] btmp = ((DataBufferByte) src
                                .getDataBuffer()).getData();
                        final MultiPixelPackedSampleModel mpp = (MultiPixelPackedSampleModel) src
                                .getSampleModel();
                        final int lineStride = mpp.getScanlineStride();
                        int inOffset = mpp.getOffset(
                                col - src.getSampleModelTranslateX(),
                                row - src.getSampleModelTranslateY());
                        if (lineStride == (int) bytesPerRow) {
                            System.arraycopy(btmp, inOffset, bpixels, 0,
                                    (int) bytesPerRow * rows);
                        } else {
                            int outOffset = 0;
                            for (int j = 0; j < rows; j++) {
                                System.arraycopy(btmp, inOffset, bpixels,
                    ...
                    if (compression == COMP_NONE) {
                        this.output.write(bpixels, 0, rows
                                * ((tileWidth + 7) / 8));
                    } else if (compression == COMP_PACKBITS) {
                        final int numCompressedBytes = compressPackBits(
                                bpixels, rows, (int) bytesPerRow, compressBuf);
                        tileByteCounts[tileNum++] = numCompressedBytes;
                        this.output.write(compressBuf, 0, numCompressedBytes);
                    } else if (compression == COMP_DEFLATE) {
                        final int numCompressedBytes = deflate(deflater,
                                bpixels, compressBuf);
                        tileByteCounts[tileNum++] = numCompressedBytes;
                        this.output.write(compressBuf, 0, numCompressedBytes);
                    }
                ...
                case 8:
                    ...
                    if (compression != COMP_JPEG_TTN2) {
                        if (useDataBuffer) {
                            ...
                            final ComponentSampleModel csm = (ComponentSampleModel) src
                                    .getSampleModel();
                            int inOffset = csm.getOffset(
                                    col - src.getSampleModelTranslateX(), row
                ...
                case 32:
                    if (dataType == DataBuffer.TYPE_INT) {
                        ...
                        int lf = 0;
                        for (int i = 0; i < size; ++i) {
                            final int value = Float.floatToIntBits(fpixels[i]);
        ...
        if (compression == COMP_NONE) {
            ...
            int totalBytes = 0;
            for (int i = 1; i < numTiles; ++i) {
                final int numBytes = (int) tileByteCounts[i - 1];
                totalBytes += numBytes;
            ...
            if (outCache == null) {
                ...
                final SeekableOutputStream sos = (SeekableOutputStream) this.output;

                // Save current position.
                final long savePos = sos.getFilePointer();
                ...
                final FileInputStream fileStream = new FileInputStream(tempFile);
                ...
                while (bytesCopied < totalBytes) {
                    final int bytesRead = fileStream.read(copyBuffer);
                    if (bytesRead == -1) {
                        break;
                    }
                ...
                final ByteArrayOutputStream memoryStream = (ByteArrayOutputStream) this.output;
                ...
                throw new IllegalStateException(
                        PropertyUtil.getString("TIFFImageEncoder13"));
            }
        }
     ...
     */
    private int getDirectorySize(final SortedSet<TIFFField> fields) {
        // Get the number of entries.
        final int numEntries = fields.size();
        ...
        for (final TIFFField field : fields) {
            ...
            final int valueSize = field.getCount()
    ...
    private void writeDirectory(final int thisIFDOffset,
            final SortedSet<TIFFField> fields, final int nextIFDOffset)
            throws IOException {
        ...
        final List<TIFFField> tooBig = new ArrayList<>();
        ...
        for (final TIFFField field : fields) {
            ...
            final int valueSize = getValueSize(field);
     ...
     */
    private static int getValueSize(final TIFFField field) {
        final int type = field.getType();
        final int count = field.getCount();
        int valueSize = 0;
        if (type == TIFFField.TIFF_ASCII) {
            for (int i = 0; i < count; ++i) {
                final byte[] stringBytes = field.getAsString(i).getBytes(); // note:
                // default
                // encoding
                // @work
                // here!
    ...
    private void writeValuesAsFourBytes(final TIFFField field)
            throws IOException {

        final int dataType = field.getType();
        ...
        case TIFFField.TIFF_BYTE:
            final byte[] bytes = field.getAsBytes();
            if (count > 4) {
                count = 4;
            }
            for (int i = 0; i < count; ++i) {
        ...
        case TIFFField.TIFF_SHORT:
            final char[] chars = field.getAsChars();
            if (count > 2) {
                count = 2;
            }
            for (int i = 0; i < count; ++i) {
        ...
        case TIFFField.TIFF_LONG:
            final long[] longs = field.getAsLongs();
    ...
    private void writeValues(final TIFFField field) throws IOException {
        ...
        case TIFFField.TIFF_SSHORT:
            final short[] shorts = field.getAsShorts();
            for (int i = 0; i < count; ++i) {
        ...
        case TIFFField.TIFF_FLOAT:
            final float[] floats = field.getAsFloats();
            for (int i = 0; i < count; ++i) {
                final int intBits = Float.floatToIntBits(floats[i]);
                writeLong(intBits);
            }
            break;

        case TIFFField.TIFF_DOUBLE:
            final double[] doubles = field.getAsDoubles();
            for (int i = 0; i < count; ++i) {
                final long longBits = Double.doubleToLongBits(doubles[i]);
        ...
        case TIFFField.TIFF_SRATIONAL:
            final long[][] rationals = field.getAsRationals();
            for (int i = 0; i < count; ++i) {
        ...
        case TIFFField.TIFF_ASCII:
            for (int i = 0; i < count; ++i) {
                final byte[] stringBytes = field.getAsString(i).getBytes();
        ...
        default:
            throw new RuntimeException(
                    PropertyUtil.getString("TIFFImageEncoder10"));
     ...
     * 
     * @param l
     *            32bits of this are written as 4 bytes
     * @throws IOException
     */
     ...
     */
    private static int compressPackBits(final byte[] data, final int numRows,
            final int bytesPerRow, final byte[] compData) {
        int inOffset = 0;
        int outOffset = 0;

        for (int i = 0; i < numRows; ++i) {
            outOffset = packBits(data, inOffset, bytesPerRow, compData,
                    outOffset);
    ...
    /**
     * Performs PackBits compression for a single buffer of data. This should be
     * called for each row of each tile. The returned value is the offset into
     * the output buffer after compression.
     */
    private static int packBits(final byte[] input, final int inputInOffset,
            final int inCount, final byte[] output, final int inputOutOffset) {
        int inOffset = inputInOffset;
        int outOffset = inputOutOffset;
        final int inMax = inOffset + inCount - 1;
        final int inMaxMinus1 = inMax - 1;

        while (inOffset <= inMax) {
            int run = 1;
            final byte replicate = input[inOffset];
            ...
            run = 0;
            final int saveOffset = outOffset;
            while (run < 128
                    && (inOffset < inMax
                            && input[inOffset] != input[inOffset + 1] || inOffset < inMaxMinus1
    ...
    private static int deflate(final Deflater deflater, final byte[] inflated,
            final byte[] deflated) {
        deflater.setInput(inflated);
        deflater.finish();
        final int numCompressedBytes = deflater.deflate(deflated);
        deflater.reset();
