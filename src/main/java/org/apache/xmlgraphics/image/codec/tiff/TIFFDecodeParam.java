...
public class TIFFDecodeParam implements ImageDecodeParam {
     ...
     * <code>setIFDOffset()</code> is never invoked then the decoder will assume
     * that the TIFF stream is at the beginning of the 8-byte image header. If
     * the directory offset is set and a page number is supplied to the TIFF
     * <code>ImageDecoder</code> then the page will be the zero-relative index
     * of the IFD in linked list of IFDs beginning at the specified offset with
     * a page of zero indicating the directory at that offset.
     */
    public void setIFDOffset(final long offset) {
        this.ifdOffset = Long.valueOf(offset);
    }

    /**
     * Returns the value set by <code>setIFDOffset()</code> or <code>null</code>
     ...
     * RGB. The default value is <code>true</code>. This flag is ignored if the
     * image data are not JPEG-compressed.
     */
