...
public class PNGRed extends AbstractRed {
	...
	private final List textKeys = new ArrayList();
	private final List textStrings = new ArrayList();

	private final List ztextKeys = new ArrayList();
	private final List ztextStrings = new ArrayList();
	...
	public PNGRed(final InputStream stream) throws IOException {
		this(stream, null);
	}
	...
	private void parse_IEND_chunk(final PNGChunk chunk) throws IOException {
		...
		for (int i = 0; i < textLen; i++) {
			final String key = (String) this.textKeys.get(i);
			final String val = (String) this.textStrings.get(i);
			textArray[2 * i] = key;
			textArray[2 * i + 1] = val;
			if (this.emitProperties) {
		...
		for (int i = 0; i < ztextLen; i++) {
			final String key = (String) this.ztextKeys.get(i);
			final String val = (String) this.ztextStrings.get(i);
			ztextArray[2 * i] = key;
