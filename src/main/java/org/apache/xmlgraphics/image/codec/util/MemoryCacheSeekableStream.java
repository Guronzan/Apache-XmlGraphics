...
public final class MemoryCacheSeekableStream extends SeekableStream {
    ...
    private long readUntil(final long pos) throws IOException {
        ...
        // First unread sector
        final int startSector = this.length >> SECTOR_SHIFT;
        ...
        for (int i = startSector; i <= sector; ++i) {
            ...
            int off = 0;
            while (len > 0) {
                final int nbytes = this.src.read(buf, off, len);
    ...
    public void seek(final long pos) throws IOException {
        if (pos < 0) {
            throw new IOException(
                    PropertyUtil.getString("MemoryCacheSeekableStream0"));
        }
        this.pointer = pos;
    }
    ...
    public int read() throws IOException {
        final long next = this.pointer + 1;
        final long pos = readUntil(next);
        if (pos >= next) {
            final byte[] buf = this.data
                    .get((int) (this.pointer >> SECTOR_SHIFT));
            return buf[(int) (this.pointer++ & SECTOR_MASK)] & 0xff;
        } else {
            return -1;
        }
    }
    ...
    public int read(final byte[] b, final int off, final int len)
        ...
        final long pos = readUntil(this.pointer + len);
        // End-of-stream
        if (pos <= this.pointer) {
            return -1;
        }

        final byte[] buf = this.data.get((int) (this.pointer >> SECTOR_SHIFT));
        final int nbytes = Math.min(len, SECTOR_SIZE
                - (int) (this.pointer & SECTOR_MASK));
        System.arraycopy(buf, (int) (this.pointer & SECTOR_MASK), b, off,
                nbytes);
        this.pointer += nbytes;
        return nbytes;
    }
