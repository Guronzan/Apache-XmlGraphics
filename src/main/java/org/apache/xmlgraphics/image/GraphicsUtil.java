...
public final class GraphicsUtil {
     ...
     */
    public static final ColorModel sRGB = new DirectColorModel(
            ColorSpace.getInstance(ColorSpace.CS_sRGB), 24, 0x00FF0000,
            0x0000FF00, 0x000000FF, 0x0, false, DataBuffer.TYPE_INT);
    /**
     * Standard prebuilt sRGB color model with premultiplied alpha.
     */
    public static final ColorModel sRGB_Pre = new DirectColorModel(
            ColorSpace.getInstance(ColorSpace.CS_sRGB), 32, 0x00FF0000,
            0x0000FF00, 0x000000FF, 0xFF000000, true, DataBuffer.TYPE_INT);
    /**
     * Standard prebuilt sRGB color model with unpremultiplied alpha.
     */
    public static final ColorModel sRGB_Unpre = new DirectColorModel(
            ColorSpace.getInstance(ColorSpace.CS_sRGB), 32, 0x00FF0000,
            0x0000FF00, 0x000000FF, 0xFF000000, false, DataBuffer.TYPE_INT);
     ...
     *
     * @param premult
     *            True if the ColorModel should have premultiplied alpha.
     * @return a ColorMdoel with Linear sRGB colorSpace and the alpha channel
     *         set in accordance with <tt>premult</tt>
     */
     ...
     *
     * @param width
     *            The desired width of the BufferedImage
     * @param height
     *            The desired height of the BufferedImage
     * @param premult
     *            The desired state of alpha premultiplied
     * @return The requested BufferedImage.
     */
    public static BufferedImage makeLinearBufferedImage(final int width,
            final int height, final boolean premult) {
        final ColorModel cm = makeLinear_sRGBCM(premult);
        final WritableRaster wr = cm.createCompatibleWritableRaster(width,
                height);
        return new BufferedImage(cm, wr, premult, null);
    }

    /**
     * This method will return a CacheableRed that has it's data in the linear
     * sRGB colorspace. If <tt>src</tt> is already in linear sRGB then this
     * method does nothing and returns <tt>src</tt>. Otherwise it creates a
     * transform that will convert <tt>src</tt>'s output to linear sRGB and
     * returns that CacheableRed.
     *
     * @param src
     *            The image to convert to linear sRGB.
     ...
     * colorspace. If <tt>src</tt> is already in sRGB then this method does
     * nothing and returns <tt>src</tt>. Otherwise it creates a transform that
     * will convert <tt>src</tt>'s output to sRGB and returns that CacheableRed.
     *
     * @param src
     *            The image to convert to sRGB.
     ...
     * <p>
     *
     * In cases where <tt>ri</tt> is not already a CacheableRed it wraps
     * <tt>ri</tt> with a helper class. The wrapped CacheableRed "Pretends" that
     * it has no sources since it has no way of inteligently handling the
     * dependency/dirty region calls if it exposed the source.
     *
     * @param ri
     *            The RenderedImage to convert.
    ...
    /**
     * An internal optimized version of copyData designed to work on Integer
     * packed data with a SinglePixelPackedSampleModel. Only the region of
     * overlap between src and dst is copied.
     *
     ...
     */
    public static void copyData_INT_PACK(final Raster src,
            final WritableRaster dst) {
        // log.info("Fast copyData");
        int x0 = dst.getMinX();
        if (x0 < src.getMinX()) {
            x0 = src.getMinX();
        }
        ...
        final int srcBase = srcDB.getOffset()
                + srcSPPSM.getOffset(x0 - src.getSampleModelTranslateX(), y0
        ...
        final int dstBase = dstDB.getOffset()
                + dstSPPSM.getOffset(x0 - dst.getSampleModelTranslateX(), y0
                        - dst.getSampleModelTranslateY());

        if (srcScanStride == dstScanStride && srcScanStride == width) {
            // log.info("VERY Fast copyData");
    ...
    public static void copyData_FALLBACK(final Raster src,
            final WritableRaster dst) {
        // log.info("Fallback copyData");
    ...
    /**
     * Copies data from one raster to another. Only the region of overlap
     * between src and dst is copied. <tt>Src</tt> and <tt>Dst</tt> must have
     ...
     * This is highly optimized for speed. There is no provision for changing
     * any aspect of the SampleModel.
     *
     * This method should be used when you need to change the contents of a
     * Raster that you do not "own" (ie the result of a <tt>getData</tt> call).
     *
     * @param ras
     *            The Raster to copy.
     ...
     * Creates a new raster that has a <b>copy</b> of the data in <tt>ras</tt>.
     * This is highly optimized for speed. There is no provision for changing
     * any aspect of the SampleModel. However you can specify a new location for
     * the returned raster.
     *
     * This method should be used when you need to change the contents of a
     * Raster that you do not "own" (ie the result of a <tt>getData</tt> call).
     ...
     *
     * @param minX
     *            The x location for the upper left corner of the returned
     *            WritableRaster.
     *
     * @param minY
     *            The y location for the upper left corner of the returned
     *            WritableRaster.
     *
     * @return A writable copy of <tt>ras</tt>
     */
    public static WritableRaster copyRaster(final Raster ras, final int minX,
            final int minY) {
        WritableRaster ret = Raster.createWritableRaster(ras.getSampleModel(),
                new Point(0, 0));
        ret = ret.createWritableChild(
                ras.getMinX() - ras.getSampleModelTranslateX(), ras.getMinY()
                - ras.getSampleModelTranslateY(), ras.getWidth(),
                ras.getHeight(), minX, minY, null);
        ...
        for (int b = 0; b < banks; b++) {
            ...
            case DataBuffer.TYPE_INT: {
                final DataBufferInt srcDBT = (DataBufferInt) srcDB;
                final DataBufferInt retDBT = (DataBufferInt) retDB;
                System.arraycopy(srcDBT.getData(b), offsets[b],
                        retDBT.getData(b), offsets[b], len);
                break;
            }
            case DataBuffer.TYPE_SHORT: {
                final DataBufferShort srcDBT = (DataBufferShort) srcDB;
                final DataBufferShort retDBT = (DataBufferShort) retDB;
                System.arraycopy(srcDBT.getData(b), offsets[b],
                        retDBT.getData(b), offsets[b], len);
                break;
            }
            case DataBuffer.TYPE_USHORT: {
                final DataBufferUShort srcDBT = (DataBufferUShort) srcDB;
                final DataBufferUShort retDBT = (DataBufferUShort) retDB;
                System.arraycopy(srcDBT.getData(b), offsets[b],
                        retDBT.getData(b), offsets[b], len);
                break;
            }
            default:
                throw new UnsupportedOperationException(
                        "unsupported data type: " + srcDB.getDataType());
            }
        }
     ...
     * interface (such as to construct a BufferedImage), but have no intention
     * of modifying the contents of the returned Raster. If you have any doubt
     * about other users of the data in <tt>ras</tt>, use copyRaster (above).
     *
     * @param ras
     *            The raster to make writable.
     ...
     * especially useful for constructing BufferedImages which require the
     * Raster to be at (0,0).
     *
     * This method should only be used if you need a WritableRaster due to an
     * interface (such as to construct a BufferedImage), but have no intention
     ...
     *         newAlphaPreMult.
     */
    public static ColorModel coerceColorModel(final ColorModel cm,
            final boolean newAlphaPreMult) {
     ...
     */
    public static ColorModel coerceData(final WritableRaster wr,
            final ColorModel cm, final boolean newAlphaPreMult) {

        // log.info("CoerceData: " + cm.isAlphaPremultiplied() +
        // " Out: " + newAlphaPreMult);
        if (!cm.hasAlpha()) {
        ...
        // log.info("CoerceData: " + wr.getSampleModel());
    ...
    /**
     * Copies data from one bufferedImage to another paying attention to the
     * state of AlphaPreMultiplied.
     *
     * @param src
     *            The source
     * @param dst
    ...
    public static void copyData(final BufferedImage src, final BufferedImage dst) {
        final Rectangle srcRect = new Rectangle(0, 0, src.getWidth(),
                src.getHeight());
     ...
     *            The source
     * @param srcRect
     *            The Rectangle of source data to be copied
     * @param dst
     *            The destination
     * @param destP
     *            The Place for the upper left corner of srcRect in dst.
     */
    public static void copyData(final BufferedImage src,
            final Rectangle srcRect, final BufferedImage dst, final Point destP) {
        ...
        // log.info("Src has: " + srcAlpha +
        // " is: " + src.isAlphaPremultiplied());
        //
        // log.info("Dst has: " + dstAlpha +
        ...
        final int y1 = y0 + srcRect.height - 1;
        ...
        if (!srcAlpha) {
            ...
            // log.info("Mult Case");
            int a, b, alpha, in;
            final int fpNorm = (1 << 24) / 255, pt5 = 1 << 23;
            for (int y = y0; y <= y1; y++) {
                pixel = srcR.getPixels(x0, y, w, 1, pixel);
                in = bands * w - 1;
                switch (bands) {
            ...
            // log.info("Div Case");
            int a, b, ialpha, in;
            final int fpNorm = 0x00FF0000, pt5 = 1 << 15;
            for (int y = y0; y <= y1; y++) {
                pixel = srcR.getPixels(x0, y, w, 1, pixel);
                in = bands * w - 1;
                switch (bands) {
            ...
            // log.info("Remove Alpha, Div Case");
            int a, b, ialpha, in, out;
            final int fpNorm = 0x00FF0000, pt5 = 1 << 15;
            for (int y = y0; y <= y1; y++) {
                pixel = srcR.getPixels(x0, y, w, 1, pixel);
                in = (bands + 1) * w - 1;
            ...
            final Rectangle dstRect = new Rectangle(destP.x, destP.y,
    ...
    public static void copyBand(final Raster src, final Rectangle insR,
            final int sBand, final WritableRaster dst, final Rectangle indR,
            final int dBand) {
        final int dy = indR.y - insR.y;
        final int dx = indR.x - insR.x;
        final Rectangle sR = insR.intersection(src.getBounds());
        final Rectangle dR = indR.intersection(dst.getBounds());
        int width, height;
    ...
    public static boolean is_INT_PACK_Data(final SampleModel sm,
            final boolean requireAlpha) {
    ...
    protected static void divide_INT_PACK_Data(final WritableRaster wr) {
        ...
        if (a <= 0) {
            ...
            final int aFP = 0x00FF0000 / a;
            pixels[sp] = a << 24 | ((pixel & 0xFF0000) >> 16) * aFP
                    & 0xFF0000
    ...
    protected static void mult_INT_PACK_Data(final WritableRaster wr) {
        ...
        if (a >= 0 && a < 255) { // this does NOT include a == 255
            // (0xff) !
            pixels[sp] = a << 24 | (pixel & 0xFF0000) * a >> 8
                    & 0xFF0000 | (pixel & 0x00FF00) * a >> 8 & 0x00FF00
     ...
     *
     *
     * // if ((count++ % 50) == 10) // org.ImageDisplay.showImage("foo: ",
     * subBI);
     *
     *
     * Graphics2D realG2D = g2d; while (realG2D instanceof
     * sun.java2d.ProxyGraphics2D) { realG2D =
     * ((sun.java2d.ProxyGraphics2D)realG2D).getDelegate(); } if (realG2D
     * instanceof sun.awt.image.BufferedImageGraphics2D) { count++; if (count ==
     * 34) { RenderedImage ri; ri =
     ...
     * org.ImageDisplay.printImage("Bar: " + count, cr, new Rectangle(75, 90,
     * 32, 32));
     *
     * org.ImageDisplay.showImage ("Foo: " + count, ri);
     * org.ImageDisplay.printImage("Foo: " + count, ri, new Rectangle(75, 90,
     * 32, 32));
     *
     * log.info("BI: " + ri); log.info("BISM: " + ri.getSampleModel());
     * log.info("BICM: " + ri.getColorModel()); log.info("BICM class: " +
     * ri.getColorModel().getClass()); log.info("BICS: " +
     * ri.getColorModel().getColorSpace()); log.info ("sRGB CS: " +
     * ColorSpace.getInstance(ColorSpace.CS_sRGB)); log.info("G2D info");
     * log.info("\tComposite: " + g2d.getComposite()); log.info("\tTransform" +
     * g2d.getTransform()); java.awt.RenderingHints rh =
     * g2d.getRenderingHints(); java.util.Set keys = rh.keySet();
     * java.util.Iterator iter = keys.iterator(); while (iter.hasNext()) {
     * Object o = iter.next();
     *
     * log.info("\t" + o.toString() + " -> " + rh.get(o).toString()); }
     *
     * ri = cr; log.info("RI: " + ri); log.info("RISM: " + ri.getSampleModel());
     * log.info("RICM: " + ri.getColorModel()); log.info("RICM class: " +
     * ri.getColorModel().getClass()); log.info("RICS: " +
     * ri.getColorModel().getColorSpace()); } }
     */

    /**
     * Extracts an alpha raster from a RenderedImage. The method tries to avoid
     * copying data unnecessarily by checking if the RenderedImage is a
     * BufferedImage which offers suitable direct methods.
     *
     * @param image
     *            the image
    ...
    public static Raster getAlphaRaster(final RenderedImage image) {
        ...
        if (image instanceof BufferedImage) {
            ...
            final WritableRaster wraster = GraphicsUtil
                    .makeRasterWritable(image.getData());
