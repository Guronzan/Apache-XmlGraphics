...
public class ImageManager {
	...
	public ImageInfo preloadImage(final String uri, final Source src)
		...
		final Iterator iter = this.registry.getPreloaderIterator();
		while (iter.hasNext()) {
			final ImagePreloader preloader = (ImagePreloader) iter.next();
	...
	private Map prepareHints(final Map hints,
			final ImageSessionContext sessionContext) {
		final Map newHints = new java.util.HashMap();
		if (hints != null) {
	...
	public Image getImage(final ImageInfo info, final ImageFlavor flavor,
			Map hints, final ImageSessionContext session)
			throws ImageException, IOException {
	...
	public Image convertImage(final Image image, final ImageFlavor[] flavors,
			Map hints) throws ImageException, IOException {
