...
public abstract class AbstractImageSessionContext implements
    ...
    public Source newSource(final String uri) throws IOException {
        ...
        if (f != null) {
            ...
            if (in != null) {
                ...
                try {
                    ...
                    log.error(
                            "Error while checking the InputStream for GZIP compression."
                                    + " Could not load image from system identifier '"
                                    + source.getSystemId() + "' ("
                                    + ioe.getMessage() + ")", ioe);
            ...
            if (directFileAccess) {
                ...
                try {
                    ...
                    log.error(
                            "Unable to create ImageInputStream for local file"
                                    + " from system identifier '"
                                    + source.getSystemId() + "' ("
                                    + ioe.getMessage() + ")", ioe);
                }
        ...
        if (imageSource == null) {
            ...
            InputStream in = null;
            try {
                in = ImageUtil.getInputStream(source);
                if (in == null && url != null) {
                    try {
                        in = url.openStream();
                    } catch (final Exception ex) {
                        log.error(
                                "Unable to obtain stream from system identifier '"
                ...
                try (final InputStream inStream = ImageUtil
                        .autoDecorateInputStream(in)) {
                    imageSource = new ImageSource(
                            createImageInputStream(inStream),
                            source.getSystemId(), false);
                } catch (final IOException ioe) {
                    log.error(
                            "Unable to create ImageInputStream for InputStream"
                                    + " from system identifier '"
                                    + source.getSystemId() + "' ("
                                    + ioe.getMessage() + ")", ioe);
                }
    ...
    protected ImageInputStream createImageInputStream(final InputStream in)
            throws IOException {
        try (final ImageInputStream iin = ImageIO.createImageInputStream(in)) {
     ...
     */
    @Override
    public Source needSource(final String uri) throws IOException {
    ...
    public void returnSource(final String uri, final Source src)
        ...
        try (final ImageInputStream in = ImageUtil.getImageInputStream(src)) {
            try {
