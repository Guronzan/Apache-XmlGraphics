...
public class ImageLoaderRawCCITTFax extends AbstractImageLoader implements
	...
	public Image loadImage(final ImageInfo info, final Map hints,
		...
		try {
			...
			if (fld != null) {
				...
				case COMPRESSION_FAX_GROUP3:
					// Note: the TIFFImage compression constants seem to be a
					// bit misleading!
					compression = TIFFImage.COMP_FAX_G3_1D; // 1D is the default
															// for Group3
		...
		in.seek(stripOffset);
		InputStream subin = new SubInputStream(ImageUtil.needInputStream(src),
				stripLength, true);
		if (fillOrder == 2) {
