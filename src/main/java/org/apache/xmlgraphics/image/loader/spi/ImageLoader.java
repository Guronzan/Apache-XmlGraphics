 ...
 * Normally, such a source will be an InputStream but can also be something
 * else.
 */
public interface ImageLoader {

    /**
     * Used if the loading penalty is negligible (image doesn't need to be
     * decoded).
     */
    int NO_LOADING_PENALTY = 0;
    /**
     * Default/Medium conversion penalty (if there's some effort to load the
     * image format)
     */
     ...
     *             if an I/O error occurs while loading the image
     */
    Image loadImage(final ImageInfo info, final ImageSessionContext session)
