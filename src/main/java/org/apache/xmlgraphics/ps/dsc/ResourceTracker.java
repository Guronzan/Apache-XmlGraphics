...
public class ResourceTracker {
    ...
    private Set<PSResource> documentSuppliedResources;
    private Set<PSResource> documentNeededResources;
    private Set<PSResource> usedResources;
    private Set<PSResource> pageResources;

    // Map<PSResource, Integer>
    private Map<PSResource, Counter> resourceUsageCounts;
     ...
     */
    public Set<PSResource> getDocumentSuppliedResources() {
     ...
     */
    public Set<PSResource> getDocumentNeededResources() {
    ...
    public void notifyResourceUsageOnPage(final PSResource res) {
        ...
        prepareUsageCounts();
        final Counter counter = this.resourceUsageCounts.get(res);
        if (counter == null) {
     ...
     */
    public void notifyResourceUsageOnPage(final Collection<PSResource> resources) {
        preparePageResources();
        for (final PSResource psResource : resources) {
            notifyResourceUsageOnPage(psResource);
        }
    }
    ...
    public void writeDocumentResources(final PSGenerator gen)
            throws IOException {
        if (this.usedResources != null) {
            final Iterator<PSResource> iter = this.usedResources.iterator();
            while (iter.hasNext()) {
                final PSResource res = iter.next();
                if (this.documentSuppliedResources == null
                        || !this.documentSuppliedResources.contains(res)) {
