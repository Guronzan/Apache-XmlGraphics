...
public class DSCCommentBoundingBox extends AbstractDSCComment {
    ...
    public void parseValue(final String value) {
        ...
        final double x1 = Double.parseDouble(iter.next());
        final double y1 = Double.parseDouble(iter.next());
        final double x2 = Double.parseDouble(iter.next());
        final double y2 = Double.parseDouble(iter.next());
        ...
        if (getBoundingBox() != null) {
            gen.writeDSCComment(
                    ...
                    new Object[] {
                            ...
                            (int) Math.ceil(this.bbox.getX()
                                    + this.bbox.getWidth()),
                            (int) Math.ceil(this.bbox.getY()
                                    + this.bbox.getHeight()) });
        } else {
